...
\usepackage{minted}  % must come before csquotes to avoid fvextra/lineno warning
\usepackage{csquotes}
...
\todo{Consider restructuring RQs into primary (RQ1--RQ3) and secondary (RQ4--RQ8) groups. Add a brief introductory paragraph explaining how these RQs were derived.}
...
\label{sec:setup}
    ...
    \end{itemize}
    UDP flooding was selected as the primary load generation method for Ethernet load testing due to its ability to saturate gigabit links reliably across all tested devices.

    In addition to the custom UDP/TCP/Layer~2 load generator, \textbf{iperf3} was used for Wi-Fi throughput testing.
    ...
    For phone-to-phone Wi-Fi load tests (Test~3), an Android iperf3 app was used to generate traffic between two smartphones connected to the DUT's Wi-Fi network.
